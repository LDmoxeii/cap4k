...
class EventRecordImpl : EventRecord {
    ...
    override val id: String = event.eventUuid

    override val eventTopic: String = event.eventType

    override val payload: Any = event.payload

    override val scheduleTime: LocalDateTime = event.createAt
    ...
    override val message: Message<Any> by lazy {
        ...
        GenericMessage(
            ...
            headers.apply {
                put(HEADER_KEY_CAP4J_EVENT_ID, event.id)
                put(
                ...
                if (this@EventRecordImpl.scheduleTime.isAfter(now)) {
                    put(HEADER_KEY_CAP4J_SCHEDULE, this@EventRecordImpl.scheduleTime.toEpochSecond(ZoneOffset.UTC))
                }
            }
        }
    }
