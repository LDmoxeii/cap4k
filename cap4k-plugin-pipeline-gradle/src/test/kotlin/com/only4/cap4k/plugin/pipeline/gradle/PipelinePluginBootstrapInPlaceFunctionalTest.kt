...
class PipelinePluginBootstrapInPlaceFunctionalTest {
    ...
    fun `cap4kBootstrap upgrades a managed minimal host root in place`() {
        ...
        val expectedDomainSlotPath = projectDir.resolve("codegen/bootstrap-slots/domain-package").toAbsolutePath().normalize()
            .toString()
            .replace("\\", "/")
        val expectedStartPackageSlotPath = projectDir.resolve("codegen/bootstrap-slots/start-package").toAbsolutePath().normalize()
            .toString()
            .replace("\\", "/")
        val expectedStartResourcesSlotPath = projectDir.resolve("codegen/bootstrap-slots/start-resources").toAbsolutePath()
            .normalize()
            .toString()
            .replace("\\", "/")
        ...
        assertTrue(buildFile.contains("modulePackage(\"start\").from(\"codegen/bootstrap-slots/start-package\")"))
        assertFalse(buildFile.contains(expectedStartPackageSlotPath))
        assertFalse(buildFile.contains(expectedStartPackageSlotPath.replace("/", "\\")))
        assertTrue(buildFile.contains("moduleResources(\"start\").from(\"codegen/bootstrap-slots/start-resources\")"))
        assertFalse(buildFile.contains(expectedStartResourcesSlotPath))
        assertFalse(buildFile.contains(expectedStartResourcesSlotPath.replace("/", "\\")))
    ...
    fun `cap4kBootstrap reruns in place and preserves host content outside managed sections`() {
        ...
        assertTrue(rerunBuild.contains("startModuleName.set(\"only-danmuku-start\")"))
        assertTrue(rerunBuild.contains("overrideDirs.from(\"codegen/bootstrap-templates\")"))
        assertFalse(rerunBuild.contains(expectedOverrideDirPath))
        assertFalse(rerunBuild.contains(expectedOverrideDirPath.replace("/", "\\")))
