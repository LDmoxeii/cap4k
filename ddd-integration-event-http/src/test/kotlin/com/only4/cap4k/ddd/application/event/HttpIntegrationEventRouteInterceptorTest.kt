...
class HttpIntegrationEventRouteInterceptorTest {
    private val interceptor = HttpIntegrationEventRouteInterceptor(
        HttpIntegrationEventRouteResolver(mapOf("content.published" to "http://localhost:8082")),
    )
    ...
    fun `eager attachment rejects a missing route before persistence`() {
        assertDoesNotThrow {
            interceptor.onAttach(ContentPublished("ok"), LocalDateTime.parse("2026-08-09T12:00:00"))
        }
        assertThrows<IllegalStateException> {
            interceptor.onAttach(ContentMissing("missing"), LocalDateTime.parse("2026-08-09T12:00:00"))
        }
    }

    @Test
    fun `pre persist defensively rejects a lazy payload route before repository save`() {
