...
- [blocking] Must every accepted Design JSON entry select a non-empty, tag-compatible artifact set containing its primary structural carrier, with secondary handlers/subscribers allowed only alongside that carrier? The recommended strict contract rejects `artifacts: []`, cross-tag families, query/capability handler-only entries, domain-subscriber-only entries, and integration-subscriber without `integration-event:inbound`; omitted `artifacts` still expands to the documented defaults. The alternative is to add a new metadata carrier so design-only or secondary-only selections remain recoverable, which creates another generated artifact and ownership surface solely to preserve otherwise incomplete inputs.
