...
- [blocking] Should Domain Event and Integration Event fields retain the same supported `defaultValue` contract as other Design JSON fields, requiring Generator and Analyzer to project it losslessly, or should event fields reject defaults and require every payload value explicitly? The recommended lossless contract renders and recovers the existing stable default-expression subset and treats whether a default is domain-correct as an authoring decision. Rejecting defaults creates a stricter historical-fact contract but narrows the shared field schema and requires event-specific validation/documentation.
