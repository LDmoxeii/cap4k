...
- [blocking] What explicit naming contract should `domain_event.eventName` use now that runtime persists `DomainEvent.value` as `eventType`? The recommended contract requires a non-blank `eventName` when `persist: true`, keeps it optional for transient Domain Events, generates `@DomainEvent(value = eventName, persist = ...)` whenever present, and makes Analyzer fail on `@BuildingBlock.eventName` versus runtime annotation conflict. Alternatives are to require an explicit name for every Domain Event, or derive an omitted name from a fixed generated-class convention; leaving a persisted event with an empty runtime `eventType` is not acceptable.
