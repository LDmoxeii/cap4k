...
- [blocking] Should `domain_service` remain a metadata-only design anchor and reject `fields`/`resultFields`, or should this audit expand it into a first-class generated operation contract? The recommended metadata-only boundary keeps generated `@DomainService class Name` as the structural anchor, leaves business methods handwritten, and fails fast on payload fields that cannot currently be represented. Expanding it now requires a new explicit operation schema covering method identity, parameters, return shape, multiplicity, and generated behavior surface; reusing the existing ambiguous `fields` key would not be sufficient.
