...
- [blocking] How should the analyzer-only metadata currently carried by `@BuildingBlock` and `@AggregateElement` be cleaned up? The recommended incremental option keeps explicit BINARY-retained annotations as the lossless code-truth carrier but moves/renames them out of `ddd-core` into a dedicated compile-time analysis-metadata contract with no runtime meaning and a compile-only project dependency. A cleaner-source but higher-cost alternative removes them and generates a sidecar skeleton index keyed by symbol FQN, changing the Analyzer truth boundary from code alone to code plus sidecar and requiring drift/ownership validation. The lowest-cost option keeps both annotations exactly where they are for now.
