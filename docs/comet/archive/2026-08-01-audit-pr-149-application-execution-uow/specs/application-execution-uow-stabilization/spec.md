...
- Domain Events are immutable local facts handled synchronously in causal frontiers. Integration Events are outbox records published only after commit. Ordinary Domain Event handlers are not mixed synchronous/asynchronous handlers.
- InvocationScope is a strict local LIFO stack identifying COMMAND, QUERY, CAPABILITY, or DOMAIN_EVENT_HANDLER. It is installed before validation/interceptors/handlers and never serialized or copied to another thread.
...
- Framework-owned async, reliable, RPC, and Integration Event boundaries capture and restore the snapshot. Reliable records preserve origin context across retry/archive; null legacy context decodes as EMPTY; decode failure prevents Handler execution; Integration Event publication is post-commit.
...
- The outer Coordinator drains non-reentrant Domain Event frontiers until there is no dirty state, unresolved intent, reliable/outbox record, pending event, or active frontier. Sibling Event and Handler order is unspecified; dispatch is fail-fast and rolls back the current transaction on Handler failure.
