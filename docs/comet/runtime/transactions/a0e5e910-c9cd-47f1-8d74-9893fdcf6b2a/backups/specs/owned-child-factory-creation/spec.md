...
- For application-side `uuid7` and `snowflake`, current typed generated-own-ID allocation shall remain authoritative, including allocation through generated owned relation mutation.
...
Given a child own ID strategy of `uuid7` or `snowflake`, the Factory caller supplies no child ID. When the child is attached through the generated owned relation, the existing typed accessor allocates an ID if missing, and a preassigned internal ID remains unchanged.
...
- No change to approved `uuid7`, `snowflake`, database identity, Unit of Work, or JPA allocation behavior.
