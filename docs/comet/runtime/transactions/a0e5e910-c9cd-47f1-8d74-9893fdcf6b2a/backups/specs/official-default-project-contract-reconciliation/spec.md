...
- 官方默认 application-side strategy 是 `uuid7`；`snowflake` 仅由独立 Snowflake starter 扩展。
- 生成器继续产生每个 eligible entity 的 typed `GeneratedOwnIdAccessor` 和每模块 catalog；Runtime 通过 `GeneratedOwnIdRegistry` 确定性发现它们。
- 不得恢复 `@ApplicationSideId`、`JpaApplicationSideIdSupport`、annotation scan、Strong ID companion 反射、generic runtime converter 或 `snowflake-long` compatibility alias。
...
- 保留 identity/integral ZERO、Snowflake Long/integral ZERO、Snowflake String/character ZERO、UUID7 String/character NIL_UUID、UUID7 native UUID/NIL_UUID 五种已支持组合。
...
- Snowflake 回归可以通过组合 fixture 验证，但 Core/JPA 默认 starter 的生产依赖树不得包含 Snowflake 实现。

## 阶段验收

### Runtime 阶段

- Core starter 在没有 Snowflake 实现时提供 UUIDv7、Identifier registry/generator 与 generated-own-ID registry。
...
- 默认项目继续只选择 Core/JPA 的 UUIDv7 基线，不选择 Snowflake 或其他高级 starter。
...
- 不创建或设计 Unique addon。
- 不改变 Snowflake、identity/version、owned relation 的既有业务语义，只约束模块迁移和 Generator refactor 不得回归。
