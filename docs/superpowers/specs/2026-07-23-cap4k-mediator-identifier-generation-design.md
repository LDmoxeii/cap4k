...
Long snowflake = Mediator.getIdentifiers().next(BuiltInIdentifierStrategies.SNOWFLAKE, Long.class);
```

Implementation agents should verify the actual Kotlin-to-Java accessor names generated by the final code. The public Java examples in tests should use the real compiled names.
