...
Generated ID types must have stable, predictable package ownership.

Aggregate-root ID types live beside their aggregate root:

```text
com.acme.content.domain.aggregates.content.ContentId
com.acme.content.domain.aggregates.media_processing_task.MediaProcessingTaskId
```

This keeps the ID in the same ubiquitous-language package as the aggregate it identifies and avoids a detached global ID bucket for aggregate-owned concepts.

Same-context aggregate references import the referenced aggregate's ID type from that aggregate package:

```kotlin
import com.acme.content.domain.aggregates.media_processing_task.MediaProcessingTaskId
```

Cross-context reference IDs declared with `@RefId` live in a shared domain identity package because they are not owned by a local aggregate:

```text
com.acme.content.domain.shared.ids.AuthorId
```

`@RefId=AuthorId` must not create `domain.aggregates.author.AuthorId`, because that would imply a local `Author` aggregate that may not exist.
...
The generator must treat Strong ID as a first-class artifact family, but it must not overclaim ownership of business artifacts.
...
For design artifacts that are generated or authored separately, the generator must support Strong ID as a resolvable field type rather than generate new business artifacts merely because an ID exists. This applies to:
...
- validators, handlers, subscribers, and mapping adapters.

In those artifacts, Strong ID support means canonical type metadata, FQN resolution, import generation, JSON/MVC binding support, and compile-safe template rendering. It does not mean every aggregate ID automatically creates commands, domain events, or integration events.
...
Recommended generated factory shape follows cap4k's existing `AggregateFactory<Payload, Aggregate>` style:
...
class ContentFactory : AggregateFactory<ContentFactory.Payload, Content> {
    ...
    override fun create(entityPayload: Payload): Content =
        Content(
            id = ContentId.new(),
            authorId = entityPayload.authorId,
            mediaProcessingTaskId = null,
            title = entityPayload.title.trim(),
        )
...
The default creation payload should not contain the aggregate root's own ID, because the factory owns creation of that new identity. Repository hydration and database loading are separate persistence concerns and do not use the aggregate factory to invent IDs.
...
Strong ID does not require an immediate Kotlin upgrade.

Cap4k already has a UUIDv7 implementation, and Strong ID's main value is the end-to-end type, generator, persistence, and boundary contract. Kotlin native UUIDv7 support is a cleanup opportunity, not a prerequisite for the Strong ID design.

The UUIDv7 generator may move to Kotlin's native UUIDv7 support when the project upgrades to a Kotlin version that provides a stable or explicitly accepted experimental API.
