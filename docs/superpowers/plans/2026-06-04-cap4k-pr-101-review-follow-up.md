...
**Files:**
- Verify: all modified files from File Map and all public docs under `docs/public`
...
```powershell
rg -n "integration event consume path|integration-event consume path|integration-event HTTP callback consume path|callback consume path|callback conversion|callback payload conversion|callback consumption adapter|callback 转成|把 callback 转成|callback.*转成.*Command|callback.*转成.*Cmd|callback.*转成.*命令|协议语义转换|inbound integration listener|用户.*实现.*adapter|手写.*consume|transport consumption/listener|Application Layer.*负责实现.*Unit of Work|Application layer.*负责实现.*Unit Of Work|负责实现.*Mediator|用户.*实现.*Mediator|Unit of Work、Repository|Unit of Work、Repository 和 Mediator 帮助 application layer 管理提交边界|inbound integration listener 消费外部事实|Integration Event consumption entry|inbound event consumption references|inbound integration listener 把 callback 或 message 转换成内部事实入口|Repository save|repository save|Repository 是否以聚合为单位保存|Repository 和 Unit of Work 负责按聚合保存|Repository 拥有聚合级持久化入口|Repository 只负责保存|借助 Repository 保存|通过 Repository 和 Unit of Work 完成保存|通过 Repository 持久化|保存交给 Repository|adapter/application 转换|adapter/application 层转换|adapter 或 application|外部协议转换|Specification 是 Domain Service 附近|Factory、Specification、Domain Service|不应保存 Repository|Application layer 负责 \\[Command\\].*Unit Of Work" docs/public
```
