...
- `ArtifactOutputKind` 当前包含 `CHECKED_IN_SOURCE`、`GENERATED_SOURCE`、`OUTPUT_ARTIFACT`。`ArtifactPlanItem.outputKind` 和 `RenderedArtifact.outputKind` 默认都是 `CHECKED_IN_SOURCE`，除非 planner 显式设置。
...
- `OUTPUT_ARTIFACT` 已在 `PipelineModels.kt` 的 enum 中定义，但当前扫描到的 built-in planners 没有显式设置 `outputKind = OUTPUT_ARTIFACT`。`ArtifactLayoutResolver` 也没有按 `OUTPUT_ARTIFACT` 分支解析路径；它只提供 Kotlin source root、generated Kotlin source root、project resource path 和 output root helper。
...
- `cap4kGenerateSources` 使用 `generatedSourceTaskConfig`，只保留 `db`、`enum-manifest` source，并把配置层业务 source generators 收窄到 `aggregate`、`aggregate-projection`。实际 runner 由 `buildSourceRunner(...)` 安装内置 planners，其中包括 `EnumManifestArtifactPlanner`（`id = "enum"`）；task 通过 plan item / exporter filtering 只导出 `GENERATED_SOURCE`，因此不会排除已安装 `enum` planner 从 `enum-manifest` input 产出的 generated source。它声明 generated source output directories，并把相关 module 的 `compileKotlin` 依赖接到 `cap4kGenerateSources`。
- `generatedSourceModuleRoles` 当前规则：`aggregate` 会为 `domain`、`adapter` 产生 generated source；`aggregate-projection` 会为 `adapter` 产生 generated source；`enum-manifest` source 由已安装的 `EnumManifestArtifactPlanner` 处理，并可为 `domain` 产生 `GENERATED_SOURCE`。
- `AggregateArtifactOutputs.checkedInKotlinArtifact` 输出到 `<moduleRoot>/src/main/kotlin`，设置 `outputKind = CHECKED_IN_SOURCE` 和 `resolvedOutputRoot = <moduleRoot>/src/main/kotlin`。aggregate factory、behavior 等需要承载手写逻辑的 skeleton 属于 checked-in source。
...
- 输出类型契约：每个 planner 都必须决定 artifact 属于 `CHECKED_IN_SOURCE`、`GENERATED_SOURCE` 还是 `OUTPUT_ARTIFACT`。构造器默认行为是 checked-in，因此非 checked-in artifact 必须显式声明。当前 built-in planner 只显式使用 `CHECKED_IN_SOURCE` 和 `GENERATED_SOURCE`。
- 路径根契约：checked-in Kotlin artifact 使用 `ArtifactLayoutResolver.kotlinSourcePath`；generated Kotlin artifact 使用 `ArtifactLayoutResolver.generatedKotlinSourcePath`；`flow`、`drawing-board` 这类 project resource 使用 `projectResourcePath`，并依赖已校验的 output root。`OUTPUT_ARTIFACT` 当前没有对应的 `ArtifactLayoutResolver` 专用 helper。
...
- 导出契约：`cap4kGenerateSources` 只能导出 `GENERATED_SOURCE`；`cap4kGenerate` 可以按完整 source-generation plan 导出 checked-in source、generated source，以及将来由 planner 明确产出的 `OUTPUT_ARTIFACT`。当前内置 planner 未产出 `OUTPUT_ARTIFACT`。
...
- 将 planner 从 `CHECKED_IN_SOURCE` 改为 `GENERATED_SOURCE` 或 `OUTPUT_ARTIFACT` 会改变 compile wiring、Gradle up-to-date inputs/outputs、conflict behavior，以及手写逻辑的所有权预期。
...
用于检查输出类型、`OUTPUT_ARTIFACT` 使用情况、generated roots、flow、drawing-board 和 analysis docs 的 PowerShell 安全显式目录命令：

```powershell
rg -n "enum class ArtifactOutputKind|CHECKED_IN_SOURCE|GENERATED_SOURCE|OUTPUT_ARTIFACT|build/generated/cap4k/main/kotlin|src-generated|flows|drawing" cap4k-plugin-pipeline-api cap4k-plugin-pipeline-core cap4k-plugin-pipeline-gradle cap4k-plugin-pipeline-generator-aggregate cap4k-plugin-pipeline-generator-design cap4k-plugin-pipeline-generator-types cap4k-plugin-pipeline-generator-flow cap4k-plugin-pipeline-generator-drawing-board docs/superpowers/analysis
```

维护本 map 时使用的补充聚焦检查：

```powershell
rg -n "outputKind = ArtifactOutputKind|ArtifactOutputKind\.GENERATED_SOURCE|ArtifactOutputKind\.CHECKED_IN_SOURCE|OUTPUT_ARTIFACT" cap4k-plugin-pipeline-generator-aggregate cap4k-plugin-pipeline-generator-design cap4k-plugin-pipeline-generator-types cap4k-plugin-pipeline-core cap4k-plugin-pipeline-api
```
...
- 如果 built-in planner 开始产出 `OUTPUT_ARTIFACT`，必须补充它的 output root、export filtering、conflict policy、Gradle task 和所有权语义。
