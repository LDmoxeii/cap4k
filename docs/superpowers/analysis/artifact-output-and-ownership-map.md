...
- `GENERATED_SOURCE` 是 build-owned、可替换的 Kotlin source，默认 root 为 `<module>/build/generated/cap4k/main/kotlin`。Aggregate/aggregate-projection、enum manifest 和需要 persistence adapter 的 value-object family 可产出此类文件；`cap4kGenerateSources` 只导出这一类型。
...
- Aggregate planners：checked-in Factory/Repository/Behavior 等 author-owned skeleton，加 build-owned Entity/Schema/Strong ID/converter/projection 等 generated source。
...
- Type planners：`enum` 与 `types-value-object`；具体 output kind 由 plan item 说明，不能仅凭 generator ID 猜测。
