...
- `generatedSourceModuleRoles` 当前规则：`aggregate` 会为 `domain`、`adapter` 产生 generated source；当 `aggregate.options["artifact.unique"] == true` 时还会包含 `application`；`aggregate-projection` 会为 `adapter` 产生 generated source；`enum-manifest` source 由已安装的 `EnumManifestArtifactPlanner` 处理，并可为 `domain` 产生 `GENERATED_SOURCE`。
