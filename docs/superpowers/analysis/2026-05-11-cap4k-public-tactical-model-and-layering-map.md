...
Generated aggregate projections, when explicitly enabled, also live in the adapter layer under `adapter.application.projections`. They are read-side persistence shapes, not application request contracts and not domain model types.

Current projection boundary:

- enable with `generators.aggregateProjection.enabled`;
- default output is JPA-flavored scalar projection only;
- object graph traversal is not generated by default;
- Jimmer use belongs in a project override of `aggregate_projection/entity.kt.peb`;
- runtime invocation still goes through existing query handlers and `Mediator.qry`, not a projection-specific mediator API.
