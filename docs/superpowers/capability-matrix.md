...
| `bootstrap.custom_template_structure` | `bootstrap` | `implemented` | `ddd-multi-module` keeps the four-role structural planner, while all concrete root/module/application files come from explicit user template overrides or slots; cap4k no longer ships an official default-project bootstrap template. | `unit`, `functional` | `DddMultiModuleBootstrapPresetProviderTest`; `PebbleBootstrapRendererTest`; `PipelinePluginBootstrapInPlaceFunctionalTest`; `PipelinePluginBootstrapGeneratedProjectFunctionalTest` | `no` | The official default project is owned by the independent GitHub Template repository, not by bootstrap resources in this repository. |
