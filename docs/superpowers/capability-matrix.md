...
| `bootstrap.start_module_baseline` | `bootstrap` | `implemented` | `ddd-multi-module` bootstrap baseline emits `domain`, `application`, `adapter`, and `start`; `start` includes fixed module build script and `StartApplication.kt`, while richer runtime wiring belongs to verification-project overrides/slots. | `unit`, `functional` | `DddMultiModuleBootstrapPresetProviderTest`; `PipelinePluginBootstrapInPlaceFunctionalTest`; `PipelinePluginBootstrapGeneratedProjectFunctionalTest` | `yes` | Current matrix coverage for this row is bounded to framework tests and generated-project functional usability. |
...
| `design.integration_event` | `design` | `implemented` | Design JSON supports `integration_event` with required `role` (`inbound` / `outbound`) and `eventName`; contracts live under `application.subscribers.integration.<role>`, inbound events generate Spring `@EventListener` subscribers, and outbound events generate contracts only. | `unit`, `functional`, `compile` | design integration event parser/canonical/planner/template tests; code-analysis extraction tests; `design-integration-event-compile-sample` | `yes` | No MQ-specific generators and no `EventSubscriber<T>` subscriber skeletons in this slice. |
