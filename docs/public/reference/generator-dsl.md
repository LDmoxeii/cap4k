...
- `com.only4.cap4k.analysis.metadata.DesignBlockMetadata` 支持 Drawing Board 普通 tag 证据恢复；
- `com.only4.cap4k.analysis.metadata.AggregateElementMetadata` 支持 metadata-dependent Flow Analysis 与 Drawing Board Aggregate element 结构证据；该结构证据不是 Design JSON。
