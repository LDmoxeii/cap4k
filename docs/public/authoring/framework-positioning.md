...
这组定位默认落在 [示例总览](examples/index.md) 的内容发布与媒体处理项目上：`Content` 负责内容生命周期，`MediaProcessingTask` 负责媒体处理生命周期，外部媒体平台通过 client 和 inbound integration event 进入边界。
...
其中 `Saga` 当前公开承诺的重点是 persisted long-running coordination、retry、recovery、compensation。它不是这次能力切片里的完整 waiting-style workflow，也不是通用的 callback-step resume runtime。
...
- 带可选只读引用上下文的读写模型分离只在确实需要导航表达时使用
- repository 仍然只感知写模型
