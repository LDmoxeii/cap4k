...
本例聚焦统一教学项目里媒体处理结果返回的推荐路径。系统先通过 `TriggerMediaProcessingCli` 把处理请求发到外部媒体平台，并记录外部任务标识。之后外部平台在处理完成或失败时，主动通过 callback 或等价的集成事件把结果推回系统；系统再把这个外部返回翻译成内部命令，推进 `MediaProcessingTask` 的状态。

这里默认已经经过一个明确交接缝：`Content` 在 `ApproveContentReviewCmd` 中完成自己的状态推进并登记领域事实后，`ContentRequiresMediaProcessingDomainEventSubscriber` 发出 `StartMediaProcessingCmd`；随后由 `StartMediaProcessingCmd.Handler` 创建 / 启动 `MediaProcessingTask`，再调用 `TriggerMediaProcessingCli`。callback 页讲的是这个交接缝之后，结果如何沿主路径返回。
...
- `TriggerMediaProcessingCli` 负责外部能力端口，不负责决定内部状态机。
...
external callback / integration event
  ...
  -> MediaProcessingTask.markSucceeded()
```

推荐读者在真实项目里至少能找到这些职责点：

- `ApproveContentReviewCmd` / `Content.approve()`：只把内容推进到审核通过，不负责创建媒体任务。
- domain-event subscriber：负责发出 `StartMediaProcessingCmd`，把内容生命周期和媒体处理生命周期接上。
- `TriggerMediaProcessingCli`：发起外部任务，请求 / 响应协议转换，错误统一。
- `StartMediaProcessingCmd.Handler`：创建 / 启动 `MediaProcessingTask`，记录外部任务标识，再把任务推进到处理中。
...
- `MarkMediaProcessingSucceededCmd`：把成功外部事实收敛为系统内部能稳定理解的命令语义。
...
- `ApproveContentReviewCmd` 一完成就让 `Content` 直接 new `MediaProcessingTask` 或直接发起 `TriggerMediaProcessingCli`，绕开单独的 `StartMediaProcessingCmd` 交接缝。
- `TriggerMediaProcessingCli` 在发起请求时顺手注册“完成后自动发布内容”的业务流程，把边界实现写成流程编排器。
- callback 路径走一套 `MarkMediaProcessingSucceededCmd`，事件总线路径又另外造一套“媒体成功同步逻辑”，没有统一命令语义。
...
- 看 `TriggerMediaProcessingCli` 是否只负责对外调用和协议转换，没有自己改内部流程状态。
- 看 `ApproveContentReviewCmd -> StartMediaProcessingCmd -> StartMediaProcessingCmd.Handler` 这条交接缝是否清楚存在，而不是由 `Content` 直接吞掉媒体任务创建。
...
- 看 `MarkMediaProcessingSucceededCmd` 是否成为 callback 主路径和其他成功回推入口的统一内部语义。
