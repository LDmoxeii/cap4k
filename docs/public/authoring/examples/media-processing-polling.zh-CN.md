...
StartMediaProcessingCmd
  ...
  -> MediaProcessingTask records ExternalMediaTaskId
...
- 看 job 是否只转换外部结果并派发 `CompleteMediaProcessingCmd`、`FailMediaProcessingCmd`、`SyncMediaProcessingProgressCmd` 等内部命令。
