...
复杂业务逻辑不应因为“实现起来方便”而越过这些 surface。比如 `Content` 是否可发布是 domain/application 问题，不应该写在 `ContentController`；媒体处理 callback 的 raw payload 是 adapter 问题，不应该进入 Aggregate；长流程补偿是 application/provider boundary 问题，不应该藏在 Capability adapter。
...
- 如果它协调多个持久化步骤，是否显式选择 provider，并记录进度、恢复和补偿。
