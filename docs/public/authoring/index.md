...
本章的运行示例是 [Reference Content Studio](../examples/reference-content-studio.md)。这个 sibling repo 展示内容草稿、审核、媒体处理 callback 和默认发布路径。authoring 页面不会重新讲完整示例代码，而是把这个项目当作共同锚点，说明从业务想法到可审查代码的作者流程。
...
3. 再设计 Clean Architecture 中的 module、Command、Query、Capability、Subscriber、Scheduled Reaction、adapter、persistence 和可选编排 provider 责任。
...
这组动作有推荐阅读顺序，但没有一次通过的承诺。后续证据可能说明一个命令边界过宽、一个 Value Object 命名不准、一段持久化编排其实只是同步反应、一个 adapter 判断混入业务规则，或者一个 plan item 暴露了 ownership 问题。authoring 的目标不是把步骤跑完，而是让这些反馈能回到正确的设计层。
