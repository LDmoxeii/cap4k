...
[Reference Content Studio](../examples/reference-content-studio.md) 的默认意图很小：内容作者创建草稿，审核通过后触发媒体处理，媒体处理成功 callback 到来后内容发布。这条路径用 Command、Domain Event Subscriber、Integration Event 和 external Capability 表达，不暗示框架内置长流程编排能力。
...
- 需要跨时间协调的高级路径必须显式选择 provider，不污染默认发布规则。

这些句子会影响后续每一层。比如“媒体处理成功是外部事实”意味着 callback 不应该伪装成 Domain Event；“默认发布可以由普通 application reaction 完成”意味着 `ContentPublicationReadyDomainEvent` 后续不需要额外编排 runtime；“高级路径必须 opt-in”意味着它不能污染默认 `Content` 发布行为。
...
这些问题决定 [Aggregate](../concepts/modeling-building-blocks/aggregate.md)、[Value Object](../concepts/modeling-building-blocks/value-object.md)、Domain Event、Integration Event、Domain Service、Capability 以及可选编排 provider 是否成立。generator 可以根据输入生成骨架，但它不会替作者决定边界。
...
判断 Aggregate 边界时，重点看一次命令处理必须保护哪些不变量。`Content` 不应该直接修改 `MediaProcessingTask`，`MediaProcessingTask` 也不应该决定 `Content` 是否可发布；它们通过 Command、Domain Event、Integration Event、Subscriber 或显式 provider-owned orchestration 协作。
...
外部能力要用业务语言命名，而不是用传输协议命名。`TriggerMediaProcessing` 和 `GetMediaProcessingStatus` 表达媒体处理能力；付款预留和权益计划也应表达为 application-facing Capability。adapter 负责协议转换，application 只看到能力合同。

如果一个外部能力调用会影响业务状态，作者还要决定它落在哪个用例边界：Command handler、Subscriber、Scheduled Reaction 或 provider-owned orchestration step。这个决定属于技术设计，但必须以业务意图和模型为基础。

## Policies

Policy 是业务规则，不是配置散点。默认发布规则、审核规则、媒体就绪规则和补偿规则都应该先在业务语言中说清，再决定落在 Aggregate behavior、Domain Service、Command handler、Subscriber 或显式编排 provider 中。

当规则需要的数据都在同一事务内可得，优先放在 Aggregate 或 Domain Service 中表达。当规则需要跨时间等待、恢复、retry 或 compensation，才考虑引入 provider-owned orchestration。不要因为流程名字听起来高级，就为简单 reaction 增加额外持久化层。
