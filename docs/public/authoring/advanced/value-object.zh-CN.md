...
在共享教学项目里，`ContentTitle`、`MediaAssetFingerprint`、`ProcessingResultSnapshot` 这类都可能是值对象候选；而 `ContentStatus`、`ReviewStatus`、`MediaProcessingStatus` 这类状态值，默认更接近枚举。
...
还要特别区分“值对象定义”和“持久化承载方式”。例如把 `ProcessingResultSnapshot` 存到一个 JSON 字段里，只说明数据库用 JSON 承载它；JSON 字段本身不是值对象定义，值对象的定义仍然来自领域语义、构造约束和等值规则。
