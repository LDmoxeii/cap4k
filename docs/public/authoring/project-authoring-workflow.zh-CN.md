...
输入源细节见 [生成输入源](generator/input-sources.zh-CN.md)。如果某个概念不能被当前输入源表达，应先标成缺口，不要伪装成已有生成能力。
...
更多 ownership 判断见 [生成 / 手写边界](generation-boundaries.zh-CN.md)。

## 5. 实现命令、查询、订阅、适配器

实现时按 [公开战术模型](tactical-model.zh-CN.md) 归位：

- 聚合行为、生命周期、规约和领域服务表达领域规则；
- 命令处理器推进写用例，加载聚合、调用行为并通过工作单元保存；
- 查询处理器默认物理落在 adapter 侧，用于读模型和投影；
- client/cli 处理器默认物理落在 adapter 侧，用于外部能力防腐；
...
- 写入是否收敛在命令处理和工作单元边界；
- query/client/cli handler 的物理落点和责任描述是否正确；
...
- design 输入暂不支持 `integration_event`；
