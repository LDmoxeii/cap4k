...
- 写入行为是否收敛在命令处理路径，而不是散落在 controller、job 或 transport glue 中
...
- 值对象、Saga、Domain Service、集成事件的生成器支持仍需要后续切片
- 分层模型和公开战术模型还会继续收敛
- design 对 command、query、client、domain_event 等已支持契约会继续打磨；`integration_event`、`value_object`、`domain_service` 仍是后续扩展位
