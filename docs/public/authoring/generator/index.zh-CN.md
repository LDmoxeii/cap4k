...
- fixed-stage pipeline 不是自由拼装工作流；作者只能声明输入、启用生成族、调整落点和模板选择，不能自由重排阶段。
- `sources { }` 先提供设计、数据库、KSP 元数据或 IR 分析输入，流水线再把这些输入折叠成一个 canonical model。
- `cap4kPlan` 先回答“这次将生成什么、落到哪里、由哪类输出根持有”。
- `cap4kGenerate` 再真正落源码产物。
- `cap4kAnalysis*` 只负责分析观察与投影导出，不是默认主生成路径。
