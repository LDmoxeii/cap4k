...
- 根 `build.gradle.kts` 中由 bootstrap 管理的配置片段
...
- 确认根 `build.gradle.kts` / `settings.gradle.kts` 的 bootstrap 管理片段与模块骨架已出现，且工程根外的内容没有被 bootstrap 当成业务代码接管。
