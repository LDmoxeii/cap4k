...
query handler、client handler 和 adapter 边界的责任说明见 [公开战术模型](tactical-model.md)。当前默认要点是：query/client handler 物理上通常在 adapter 侧，但实现的是 application 请求契约。

## 这一层负责什么

- 提供开放服务入口实现，把外部同步请求转换成内部命令或查询。HTTP controller、RPC endpoint、gRPC service 都只是实现形态。
- 提供外部事实入口实现，把会推进状态的 callback、message listener、inbound integration event 转换成内部命令，把纯观察类输入转换成查询或明确的 application entry point。
...
- 提供 `TriggerMediaProcessingCli`、`GetMediaProcessingStatusCli` 这类外部能力边界实现，负责协议转换、异常转换、请求发送与结果读取。
...
- 外部回调或集成事件转内部命令，例如把媒体平台成功回调转换成 `MarkMediaProcessingSucceededCmd`。
...
- `TriggerMediaProcessingCli`、`GetMediaProcessingStatusCli` 的实现，用来封装发起处理、查询处理状态、统一第三方错误。
...
- `Content` 或 `MediaProcessingTask` 的业务真相，不在 controller、job、repository adapter、client 实现里决定状态机规则。
- 聚合生命周期规则，例如“什么时候允许发布”“什么时候允许处理失败后重试”。
- 跨层偷塞 application / domain 逻辑，例如开放服务入口里串完整业务流程，或外部能力 client 里替聚合决定下一状态。
- 把外部 DTO、数据库结构、第三方状态码直接当成领域对象传播到内部。
- 把 inbound integration event 伪装成 domain event。它仍然是外部事实入口，推进状态前必须转换成内部 command。
- 在 adapter 中决定对外集成事件，或绕过 `Mediator.events.attach(...)` 调用任何直接发布/低层发送 API。
...
- callback、inbound integration event 与 polling 可以各有自己的入口文件，但进入应用层之后应该汇合到同一套内部命令语义。外部事件不改名成领域事件，外部 payload 也不越过 adapter / application 收敛点。
...
- integration listener 把外部媒体事件包装成 `MediaProcessingCompletedDomainEvent` 后交给领域订阅器，掩盖它其实来自外部系统。
...
- [内容发布与处理示例项目总览](examples/reference-project-overview.md)：先看统一教学项目里有哪些外部入口、内部命令和聚合边界需要被 adapter 接住。
- [内容草稿到发布主链路](examples/content-draft-to-publish.md)：看 Web / UI 请求如何只翻译成 `CreateContentDraftCmd`、`SubmitContentForReviewCmd`、`ApproveContentReviewCmd`、`PublishContentCmd`，而不直接改 `Content`。
- [媒体处理 callback 主路径](examples/media-processing-callback.md)：看 callback controller、integration listener、`IntegrationEventSubscriber`、`TriggerMediaProcessingCli` 各自该停在哪一层。
- [媒体处理 polling 备用路径](examples/media-processing-polling.md)：看 polling job 为什么只该承担外部任务查询和内部命令转换，而不该升级成主流程编排器。

## 最低验证与审计检查点

- 所有外部输入是否都先被转换成内部命令 / 查询，再进入应用层推进。
- inbound integration event 是否仍按外部事实入口审计，没有伪装成 domain event。
- adapter 是否只承担边界与适配职责，而没有自己决定 `Content` 或 `MediaProcessingTask` 的业务状态机。
- callback 主路径和 polling 备用路径是否都落到一致的内部命令边界，没有长出两套事实来源。
- `TriggerMediaProcessingCli`、`GetMediaProcessingStatusCli`、controller、job、repository adapter 里是否没有重复编写领域规则。
