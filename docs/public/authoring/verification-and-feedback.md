...
- application tests：Command orchestration、zero-trust validation、Unit of Work、Subscriber reaction、Saga recovery/compensation、Scheduled Reaction 和 external capability semantics。
- adapter tests：Controller mapping、payload conversion、client-handler error conversion、query output shape、callback conversion。
- start smoke tests：runtime assembly、module wiring、HTTP happy path 和 callback wiring。
...
这些 examples 帮助读者观察外部入口、callback consume path、状态返回和 happy path。它们不能替代内层 focused tests，也不能把 HTTP response shape 当成领域模型来源。HTTP evidence 发现的问题，应回到 adapter mapping、application command 或 domain behavior 分别处理。
