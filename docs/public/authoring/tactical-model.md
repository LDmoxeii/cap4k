...
2. 通过 `Mediator.repositories` 加载聚合，或通过 `Mediator.factories` 创建聚合根。
   读取默认是 detached/read-only；只有后续要修改并通过 `Mediator.uow.save()` 落库的目标聚合，才显式传 `persist = true`。
...
- 命令处理器通过 `Mediator.repositories.findOne`、`findFirst`、`find` 等加载聚合根；
  默认读取不进入 UoW；真正要修改的目标聚合应显式使用 `persist = true`。
...
拆行为、拆事件、广播消费要分开判断：业务行为本身不同，优先拆聚合行为；完成的业务事实不同，才拆领域事件；只是消费者不同，不自动拆事件。一个领域事实可以被多个独立 listener 消费，但 listener 只负责路由后续 command，不成为流程所有权判定中心。listener 侧可以用事件快照做便宜过滤，真正的写入资格仍由 command 重新加载状态并校验前置条件。
...
- 不要给非聚合根生成工厂。
- 不要因为一个领域事实有多个消费者就自动拆事件，也不要把中央 listener 分支当成写入信任边界。
