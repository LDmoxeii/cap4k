...
- `content` 表标记 `@AggregateRoot=true;`，承载 `Content` 聚合根。
- `media_processing_task` 表标记 `@AggregateRoot=true;`，承载 `MediaProcessingTask` 聚合根。
- `paid_publication_task` 表标记 `@AggregateRoot=true;`，承载 paid publication 的持久化协调状态。
- `content.release_policy` 使用 `@T=ReleasePolicy;`，连接 enum manifest。
- `media_processing_task.result_snapshot` 使用 `@T=MediaProcessingResultSnapshot;`，连接 JSON-backed Value Object。
