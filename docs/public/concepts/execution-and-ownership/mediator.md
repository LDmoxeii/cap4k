...
当 controller、console entry、subscriber、job 或 adapter entry 需要触发应用层行为时，可以通过 `Mediator.commands`、`Mediator.queries` 或 `Mediator.capabilities` 进入明确分类。Mediator 不暴露 generic `requests` 逃生口；Command、Query 与 Capability 各自保留独立策略。
