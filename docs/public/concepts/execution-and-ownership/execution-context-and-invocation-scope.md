...
`askAsync()`、`callAsync()`、可靠 worker 和框架 transport adapter 会自动捕获、安装并清理 ExecutionContext。Event Handler scope 还会跟踪当前 Handler 启动的 `askAsync()` / `callAsync()`：方法体返回后等待全部任务，任一失败都会使 Handler 完成失败。用户自己的 executor、`CompletableFuture.supplyAsync`、Reactor、coroutine 或 SDK callback 不会被无感接管，也不会被 Handler scope 自动等待；如需传播上下文必须显式使用框架提供的 task/executor wrapper。传播内容不包括 Unit of Work、EntityManager、Spring transaction、InvocationScope、事件运行时状态或任意 ThreadLocal。

InvocationScope 是严格 LIFO 的本地 guard。它实现以下调用边界：Command 可以同步嵌套 Command、调用 Capability，但不能调用 Query；Query 可以同步嵌套 Query、调用 Capability，但不能发送 Command 或在 Query 内启动 `askAsync()`；Capability 只能组合 Capability，不能进入 Repository、Factory、Command、Query 或 UoW；Event Handler 可以同步发送或可靠调度 Command、同步查询 Query、同步调用 Capability，也可以用 `askAsync()` / `callAsync()` 并行收集信息，但不直接操作 Repository 或 Factory。需要改变本地状态的最终动作仍通过 Command 表达。
