...
cap4k 不再提供专用 Aggregate Specification carrier，也不会在 Unit of Work 保存阶段自动调用 Specification。需要显式业务判断时，先确认它应该是 Aggregate / Value Object 不变量还是 Domain Service decision；物理唯一性、非空和引用完整性等持久化约束继续由数据库 schema 保底。
