...
Domain Service 表达不自然归属于单个 Aggregate、Entity 或 Value Object 的领域决策。它仍然是 domain layer 的业务语言，不是 application service，也不是外部能力 adapter。它适合承载需要多个领域对象共同参与，但又不应强行塞进某个 Root 的判断。
...
cap4k 不再提供专用 Aggregate Specification carrier，也不会在 Unit of Work 稳定化阶段自动调用 Specification。需要显式业务判断时，先确认它应该是 Aggregate / Value Object 不变量还是 Domain Service decision；物理唯一性、非空和引用完整性等持久化约束继续由数据库 schema 保底。
