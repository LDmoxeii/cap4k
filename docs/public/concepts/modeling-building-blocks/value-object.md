...
在 cap4k 中，Value Object 位于 domain layer，通常被 Aggregate 或 Entity 持有。`types.valueObjectManifest` 是当前 Value Object 生成入口，可表达 shared 或 aggregate-owned 的值对象、字段、包路径和 JSON-backed converter。generator 可以生成 data class、converter 和 building-block metadata；值对象的业务命名、字段语义、校验规则、默认值取舍和使用边界仍需要人工设计。
