...
Application layer 也不应该把领域不变量挪到 handler 里。Command handler 可以做 zero-trust validation、读取、调用 Aggregate 行为和保存，但内容是否可发布、状态如何转换、Domain Event 何时出现，应由 domain layer 的手写逻辑表达。
...
手写逻辑应该落在 Command/Query handler、Subscriber reaction、Saga step、Scheduled Reaction 和 external capability request 使用处。它负责加载目标 Aggregate、调用 domain behavior、使用 Unit of Work 保存、通过 Mediator 分发内部请求、表达 no-op 或 retreat 原因，并在需要时调用 external capability contract。
...
审核 application layer 时，先看 Command/Query separation 是否清晰，再看写入路径是否通过 Aggregate 和 Unit of Work 完成。随后检查 Subscriber、Saga 和 Scheduled Reaction 是否表达 application reaction，而不是隐藏 adapter protocol。最后确认 external capability requests 是业务能力语义，HTTP payload details 和 client-handler implementation 没有进入 application layer。
