...
Contract 不是第五个业务层，而是跨服务 published language 的依赖叶子。它承载 [Endpoint](../concepts/execution-and-ownership/endpoint.md) Request/Response、Integration Event payload 以及轻量 `cap4k-contract-api` 依赖；Provider/Consumer 可以单向依赖它，但 contract 不依赖 domain、application、adapter、start、Spring 或 transport implementation。
