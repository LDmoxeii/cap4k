...
Adapter layer 可以依赖 application/domain 暴露的入口和契约。Controller 把 HTTP 请求转换成 Command 或 Query，API Payload 描述协议字段，query adapter 组织读取输出，client-handler 把 application 的 external capability request 转换成外部调用，inbound integration listener 把 callback 或 message 转换成内部事实入口，persistence adapter 处理存储协议。
...
Adapter layer 不允许成为业务真相层。Controller、query handler、client-handler、inbound listener 和 persistence adapter 可以处理 mapping、技术错误、外部协议差异和返回格式，但不应该把 Aggregate invariant、发布规则、paid publication eligibility 或 Saga business step decision 藏在协议转换代码里。
