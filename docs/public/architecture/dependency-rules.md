...
Clean Architecture 的依赖规则可以简化成一句话：越靠近业务真相的代码越独立，越靠近外部世界的代码越负责转换和装配。cap4k 项目中，dependency-leaf contract role 与 domain、application、adapter、start 四层可以协作，但源码依赖和业务责任不能随意反向流动。

## Allowed Directions

Contract module 是发布语言依赖叶子。它只包含 Endpoint Request/Response、Integration Event payload 以及 `cap4k-contract-api` 这样的轻量 marker/annotation 依赖；不得依赖本服务 domain、application、adapter、start、Spring、persistence 或 transport implementation。Provider application/adapter 与 Consumer application/transport adapter 可以单向依赖 contract。
...
审查依赖方向时，可以按五个问题走：contract 是否保持依赖叶子，domain 是否独立，application 是否只编排用例，adapter 是否只做 protocol conversion，start 是否只做 assembly。若发现 inner layer 需要 import outer layer 类型，优先把外部细节转换成内部请求或领域事实，而不是放宽依赖规则。

生成骨架也要按同样规则审查。generated skeleton 可以给出 Command、Query、Capability、Endpoint、Subscriber、api payload、domain event、integration event 或 module wiring 的稳定位置；handwritten logic 需要落在对应层的可维护位置。generator 是组织工具，不是业务决策来源。
