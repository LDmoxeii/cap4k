...
运行本地流程时，`.http` 返回值是主要观察面。默认路径的关键状态是 `contentStatus = PUBLISHED` 和 `processingStatus = SUCCEEDED`。

如果只做静态阅读，可以直接看 `ContentStudioHappyPathHttpSmokeTest` 和 `MediaProcessingCallbackIntegrationEventSmokeTest`。这些测试把 README 中的手工路径和 runtime 行为放在同一组证据里。
