...
`ReleasePolicy` 仍然在 `Content` 聚合本地 enum package 中。manifest 不额外表达 shared / local；package path 本身就是边界。默认发布路径只发布 `ReleasePolicy.IMMEDIATE` 内容。
...
`types.valueObjectManifest` 和 `types.enumManifest` 是 type input path，不是业务决策引擎。它们可以让 generator 输出稳定的 value object、enum、converter 和 metadata；但哪些状态代表发布准备、什么时候发布、外部结果如何解释，仍然由 `ContentBehavior.kt`、`PublishContentCmd` 和 Subscriber/Capability 的手写逻辑负责。
