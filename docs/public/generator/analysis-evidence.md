...
analysis generation 使用 `sources.irAnalysis.inputDirs` 选择 IR analysis input。当前 compiler analysis output root 是：
...
`cap4kAnalysisGenerate` 生成 analysis/observation outputs。当前 public docs 主要讨论两类：
...
Must avoid: 不要暗示 analysis outputs 是 source skeletons；不要把 flow/drawing-board 放进 cap4kPlan/cap4kGenerate；不要暗示 analysis evidence 会证明业务规则自动正确；不要使用旧的 irAnalysis 启用开关 wording
