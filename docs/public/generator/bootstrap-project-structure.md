...
cap4k 制品不再内置一份“官方默认项目”Bootstrap 模板。执行 Bootstrap 前必须显式提供团队自己的 template override；slots、managed block 和 conflict policy 继续保护这些输出。Bootstrap 不会替代业务建模，之后仍然需要作者完成业务意图、Aggregate、Command、Query、Event、external capability、Saga、schema、`design/design.json` 和 type manifests。
