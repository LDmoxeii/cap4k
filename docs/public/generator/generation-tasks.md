...
这些 skeleton 可以成为长期维护的入口，例如 Command、Query、Capability、Endpoint contract、Subscriber、Repository adapter、Factory、Creation、Domain Event、Integration Event contract、Value Object 或 enum。所有 checked-in skeleton 都固定使用 `SKIP`；cap4k 不承诺后续同步最新 template。
