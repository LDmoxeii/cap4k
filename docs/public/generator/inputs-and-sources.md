...
- `@RefId=<TypeName>` 这类当前上下文外部概念引用身份。
...
`design/design.json` 是 source generation 的主要 building-block 输入。当前常用 tag 包括：
...
analysis generation 使用独立输入面，不和普通 source generation 混在一起。当前 DSL selection 是 `sources.irAnalysis.inputDirs`。
