...
- `cleanArchitecturePlacement`: application-facing Capability call, adapter Capability Handler, Domain behavior receiving translated result.
- `cap4kCarriers`: Capability call, Command handler, optional Query/read model for status.
- `generatorInputPlan`: expected capability/capability-handler skeletons and any required design input or exception.
...
- `rollbackTriggers`: provider terms in Domain, capability-first write split, or missing skeleton input.

## Generator Input Implications

- Use generator-supported capability/capability-handler skeletons where cap4k can express the external capability.
- Record any handwritten structural exception in technical design before creating new Capability or Handler paths.
- Keep provider SDK DTOs and transport status fields out of generator inputs meant to represent business language.

## Handwritten Slots

- Application-facing Capability contract names the business need.
...
- Write paths enter Command or application use case before external Capability invocation changes business state.
- No entry code performs a Capability call first and Command delegation later for the same write use case.

## Rollback Target

Return to tactical modeling when the outside dependency is not clearly a capability, or to technical design/generator inputs when the Capability skeleton, provider mapping boundary, or idempotency policy is unclear.
