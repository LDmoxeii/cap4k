...
2. Stop and return to `cap4k-generator-inputs` when the subscriber shell, inbound integration event, domain-event shell, `*Cmd.kt`, `*Qry.kt`, `*QryHandler.kt`, `*CliHandler.kt`, client, or payload surface is generator-capable but missing, including generated handler surfaces.
...
4. Stop and return to `cap4k-technical-design` when the missing piece is the design entry, DDL contract, `types.enumManifest`, `types.valueObjectManifest`, or `types.registryFile` entry.
5. Stop and return to `cap4k-generator-inputs` when generation is blocked by missing analysis/setup inputs that the subscriber shell needs.
