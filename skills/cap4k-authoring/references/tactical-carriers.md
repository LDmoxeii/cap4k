...
- Endpoint is one transport-neutral published Actor operation with explicit stable `operationName` and Request/Response. Business code calls it through `Mediator.endpoints`; a Provider implementation or future Consumer proxy is a local `EndpointHandler`, never a directly called client service.
